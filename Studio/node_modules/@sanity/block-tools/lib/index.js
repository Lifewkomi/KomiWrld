...
var flatten = require("lodash/flatten.js"), isEqual = require("lodash/isEqual.js"), uniq = require("lodash/uniq.js"), getRandomValues = require("get-random-values-esm");
...
function normalizeBlock(node, options = {}) {
  ...
  return block.children = block.children.reduce(
    (acc, child) => {
      ...
      return previousChild && isPortableTextSpan(child) && isPortableTextSpan(previousChild) && isEqual__default.default(previousChild.marks, child.marks) ? (lastChild && lastChild === child && child.text === "" && block.children.length > 1 || (previousChild.text += child.text), acc) : (acc.push(child), acc);
    },
    []
  ).map((child, index) => {
    if (!child)
