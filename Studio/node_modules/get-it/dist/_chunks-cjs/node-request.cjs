"use strict";
var decompressResponse = require("decompress-response"), follow = require("follow-redirects"), http = require("http"), https = require("https"), progressStream = require("progress-stream"), qs = require("querystring"), stream = require("stream"), url = require("url"), tunnel = require("tunnel-agent");
...
const isReactNative = typeof navigator > "u" ? !1 : navigator.product === "ReactNative", defaultOptions = { timeout: isReactNative ? 6e4 : 12e4 }, processOptions = function(opts) {
  ...
  if (options.timeout = normalizeTimeout(options.timeout), options.query) {
    const { url: url2, searchParams } = splitUrl(options.url);
    for (const [key, value] of Object.entries(options.query)) {
      ...
      search && (options.url = `${url2}?${search}`);
    }
  }
...
function splitUrl(url2) {
  ...
  if (qIndex === -1)
    return { url: url2, searchParams: new URLSearchParams() };
  const base = url2.slice(0, qIndex), qs2 = url2.slice(qIndex + 1);
  if (!isReactNative)
    return { url: base, searchParams: new URLSearchParams(qs2) };
  if (typeof decodeURIComponent != "function")
    throw new Error(
      "Broken `URLSearchParams` implementation, and `decodeURIComponent` is not defined"
    );
  const params = new URLSearchParams();
  for (const pair of qs2.split("&")) {
...
const isStream = (stream2) => stream2 !== null && typeof stream2 == "object" && typeof stream2.pipe == "function", adapter = "node";
...
const reduceResponse = (res, reqUrl, method, body) => ({
  ...
  request.once("error", (err) => callback(new NodeRequestError(err, request))), options.timeout && timedOut(request, options.timeout);
...
function getProgressStream(options) {
  ...
  return { bodyStream: (bodyIsStream ? options.body : stream.Readable.from(options.body)).pipe(progress), progress };
}
...
exports.NodeRequestError = NodeRequestError;
exports.adapter = adapter;
exports.httpRequester = httpRequester;
exports.processOptions = processOptions;
exports.validateOptions = validateOptions;
//# sourceMappingURL=node-request.cjs.map
